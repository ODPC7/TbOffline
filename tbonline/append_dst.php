<?PHP
//+++ ตรวจสอบการเข้าระบบ
if ( ! isset($_SESSION["sensitivity"]) or $_SESSION["sensitivity"] !="1")	{
	print "<script>
...
if (isset($_GET["dst"]) and isset($_GET["culno"]) and isset($_GET["method"]))	{
	$ptt_name =$_GET["dst"];
	$culno =$_GET["culno"];
	$method =$_GET["method"];
	$m =$_GET["tt"];
	...
	for ($i=1; $i<=$m; $i++){
		...
		mysql_query("insert into dst(fname, cul_no, anal_method, drug, result, test_date, datereport, report_by)
				values ('$ptt_name', '$culno', '$method', '$drug', '$result', '$testdate', '$datereport', '$reporter')	");
	}

	print "<script>	window.location='./append_dst.php?dst=$ptt_name';		</script>	";
...
if (isset($_SESSION["user_name"]))	{
	...
	if (! isset($_POST["cul_no"]) and ! isset($_POST["anal_method"])){
		print "	<form class='form-horizontal' role='form' name='dst_form' method='post' action='./append_dst.php?dst=$profile_name'>	";
	}
		...
		print "	<div class='form-group'>
				...
				<div class='col-sm-9'>
					<select class='form-control' name='anal_method' required >
						<option selected value=''>โปรดระบุ ..</option>
		";
		$sql_method =mysql_query("select * from method where method_group='dst' order by num asc");
...
//+++	ช่วงเพิ่มตัวยาเข้าฐานตามวิธีการตรวจวิเคราะห์
	...
	if (isset($_POST["cul_no"]) and isset($_POST["anal_method"])){
		...
		print "	<form class='form-horizontal' role='form' name='seledrug_form' method='post' 
			action='./append_dst.php?dst=$profile_name&culno=$culno&method=$method&tt=$m'>	";
		print "		<div class='form-group'>
				<label for='cul_no' class='col-sm-3 control-label'>Culture no.</label>
				<div class='col-sm-9'>
