<?PHP
	if (isset($_GET["exp_type"]) and $_GET["exp_type"]=="file")	{
		...
		header('Content-Disposition: attachment; filename=report4.xls');
...
print "<th style='text-align:center;color:#2F70A8; vertical-align:middle;' rowspan='2'>ชนิดตัวอย่างส่งตรวจ</th>";
...
$final_row =0;
$sql_specimen =mysql_query("select distinct specimen from culture where date_in>='$start_date' and date_in<='$end_date' order by specimen ");
while ($list_specimen =mysql_fetch_array($sql_specimen))	{
	$specimen =$list_specimen["specimen"];

	print "<tr>";
	print "<td style='text-align:left;'>$specimen</td>";
	...
	while ($list_offssj =mysql_fetch_array($sql_offssj))	{
		...
		$sql =mysql_query("select count(c.fname)as count_pt from culture c, hospcode h
			where c.froms=h.off_name1 and c.specimen='$specimen' and h.off_ssj='$off_ssj'  
...
<p><b>นิยาม : จำนวนตัวอย่างที่ส่งตรวจและได้ลงทะเบียน แยกตามชนิดตัวอย่างที่ส่งตรวจของแต่ละจังหวัด
