<?PHP
	if (isset($_GET["exp_type"]) and $_GET["exp_type"]=="file")	{
		...
		header('Content-Disposition: attachment; filename=report5.xls');
...
$sql_cul_result =mysql_query("select distinct result from culture order by result");
$row_result =mysql_num_rows($sql_cul_result);
	$row_result =($row_result+1);

print "<p><b>นิยาม : จำนวนตัวอย่างที่ส่งตรวจและได้ลงทะเบียน แยกตามประเภทผู้ป่วย และผลการเพาะเชื้อ ของแต่ละจังหวัด
...
print "<th style='text-align:center;color:#2F70A8;' colspan='$row_result'>ผลการตรวจเพาะเชื้อ ' รวมทุกจังหวัด ' ตั้งแต่วันที่ $start_date ถึงวันที่ $end_date</th>";
print "</tr>";
print "<tr class='success'>";

while ($list_result =mysql_fetch_array($sql_cul_result))	{
...
while ($list_pttype =mysql_fetch_array($sql_pttype))	{
	...
	while ($list_result =mysql_fetch_array($sql_cul_result))	{
		...
		$sql =mysql_query("select count(*)as count_pt from culture 
			where result='$result' and pttype='$pttype' and date_in>='$start_date' and date_in<='$end_date'	");
...
while ($list_prov =mysql_fetch_array($sql_offssj))	{
	...
	print "<tr class='success'>";
	print "<th style='text-align:center;color:#2F70A8; vertical-align:middle;' rowspan='2'>ประเภทผู้ป่วย</th>";
	print "<th style='text-align:center;color:#2F70A8;' colspan='$row_result'>ผลการตรวจเพาะเชื้อของจังหวัด ' $off_ssj ' ตั้งแต่วันที่ $start_date ถึงวันที่ $end_date</th>";
	print "</tr>";
	print "<tr class='success'>";
	...
	while ($list_pttype =mysql_fetch_array($sql_pttype))	{
		...
		while ($list_result =mysql_fetch_array($sql_cul_result))	{
			...
			$sql =mysql_query("select count(c.fname)as count_pt from culture c,hospcode h
				where c.froms=h.off_name1 and h.off_ssj='$off_ssj' and c.result='$result' 
					and c.pttype='$pttype' and c.date_in>='$start_date' and c.date_in<='$end_date'	");
			$show_count =mysql_fetch_array($sql);	
				$count_pt =$show_count["count_pt"];
...
<p><b>นิยาม : จำนวนตัวอย่างที่ส่งตรวจและได้ลงทะเบียน แยกตามประเภทผู้ป่วย และผลการเพาะเชื้อ ของแต่ละจังหวัด
	<br>N/A หมายถึง ไม่ได้ระบุมาในใบส่งตรวจ
	<br>ตัวอย่างที่ได้ลงทะเบียน หมายถึง ลงทะเบียนตรวจในระบบ TB Online ของ สคร.7 จังหวัดขอนแก่น	</b>
	<br>(บางตัวอย่างที่ส่งตรวจแต่ยังไม่ได้ลงทะเบียน อาจเนื่องเพราะเจ้าหน้าที่ตรวจวิเคราะห์ยังไม่ได้ลงทะเบียน อาจทำให้ตัวเลขเปลี่ยนแปลงได้)</p>
