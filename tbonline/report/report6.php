<?PHP
	if (isset($_GET["exp_type"]) and $_GET["exp_type"]=="file")	{
		...
		header('Content-Disposition: attachment; filename=report6.xls');
...
print "<th style='text-align:center;color:#2F70A8;' colspan='$row_result'>ผลการตรวจ Gene-Xpert FLDST ' รวมทุกจังหวัด ' ตั้งแต่วันที่ $start_date ถึงวันที่ $end_date</th>";
print "</tr>";
print "<tr class='success'>";
...
while ($list_pttype =mysql_fetch_array($sql_pttype))	{
	...
	while ($list_result =mysql_fetch_array($sql_mol_result))	{
		...
		$sql =mysql_query("select count(dst.fname)as count_pt from culture c,dst
			where c.fname=dst.fname and c.cul_no=dst.cul_no and dst.anal_method='Gene-Xpert FLDST' and
...
while ($list_prov =mysql_fetch_array($sql_offssj))	{
	...
	print "<tr class='success'>";
	print "<th style='text-align:center;color:#2F70A8; vertical-align:middle;' rowspan='2'>ประเภทผู้ป่วย</th>";
	print "<th style='text-align:center;color:#2F70A8;' colspan='$row_result'>ผลการตรวจ Gene-Xpert FLDST จังหวัด ' $off_ssj ' ตั้งแต่วันที่ $start_date ถึงวันที่ $end_date</th>";
	print "</tr>";
	print "<tr class='success'>";
		...
		while ($list_result =mysql_fetch_array($sql_mol_result))	{
			...
			$sql =mysql_query("select count(dst.fname)as count_pt 
				...
				where h.off_name1=c.froms and h.off_ssj='$off_ssj' and c.fname=dst.fname 
					and c.cul_no=dst.cul_no and dst.anal_method='Gene-Xpert FLDST' 
